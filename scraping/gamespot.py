...
def standardize_url(domain, path, protocol='https'):
    ...
    domain_from_path = (match.groups()[3] if match.groups()[3] else '') + (match.groups()[4] if match.groups()[4] else '')
    path = match.groups()[6]
    
    protocol = protocol_from_path if protocol_from_path else protocol
    domain = domain_from_path if domain_from_path else domain    
    standardized_url = (protocol if protocol else '') + "://" + (host if host else '') + (domain if domain else '') + "/" + (path if path else '')
    if standardized_url[-1] != "/":
        standardized_url += "/"
    return standardized_url
    
def random_gamespot_post():
